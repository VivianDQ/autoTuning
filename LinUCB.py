...
class LinUCB:
    ...
    def linucb_fixed_explore(self, explore, lamda=1):
        T = self.T
        d = self.data.d
        regret = np.zeros(T)
        xr = np.zeros(d)
    ...
    def linucb_theoretical_explore(self, lamda=1):
        T = self.T
        d = self.data.d
        regret = np.zeros(T)
        xr = np.zeros(d)
        ...
        for t in range(T):
            ...
            # explore = math.sqrt( 0.5*math.log(2*(t+1)**2*K) )
            explore = 0.5 * math.sqrt(2*math.log( T* math.sqrt(np.linalg.det(B))/lamda**(d/2) ) ) + math.sqrt(lamda)
            # math.sqrt(2*math.log( T* (np.linalg.det(B))/lamda**(d/2) ) ) + math.sqrt(lamda)
            for arm in range(K):
                ucb_idx[arm] = feature[arm].dot(theta_hat) + explore * math.sqrt( feature[arm].T.dot(B_inv).dot(feature[arm]) )
            pull = np.argmax(ucb_idx)
            observe_r = self.data.random_sample(t,pull)
    ...
    def linucb_auto_advanced(self, explore_rates, lamda=1):
        T = self.T
        d = self.data.d
        regret = np.zeros(T)
        xr = np.zeros(d)
