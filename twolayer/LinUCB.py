...
class LinUCB:
    ...
    def linucb_auto(self, explore_rates, lamdas):
        T = self.T
        d = self.data.d
        regret = np.zeros(T)
        xr = np.zeros(d)
    ...
    def linucb_op(self, explore_rates, lamdas):
        T = self.T
        d = self.data.d
        regret = np.zeros(T)
        xr = np.zeros(d)
        ...
        for t in range(T):
            ...
            explore = explore_rates[index]
            slam, flam, index_lam = op_tuning(slam, flam, observe_r, index_lam)
            lamda = lamdas[index_lam]
        return regret