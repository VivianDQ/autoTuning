...
for i in range(rep):
    ...
    if 'lin' in algo:
        ...
        theta = np.random.uniform(lb, ub, d)
        fv = np.random.uniform(lb, ub, (T, K, d))
        context_nn = data_generator.context_nn
        bandit = context_nn(K, T, d, sigma, true_theta = theta, fv=fv)
    ...
    if algo == 'linucb':
        ...
        algo_class = NeuralUCB(bandit, T)
    
    fcts = {
        k: getattr(algo_class, algo+methods[k]) 
        for k,v in methods.items()
    }
    reg_theory += fcts['theory'](explore, eta)
    reg_op += fcts['op'](J, eta)
    reg_auto += fcts['auto'](J, eta)
    reg_auto_3layer += fcts['auto_3layer'](J, etas)
